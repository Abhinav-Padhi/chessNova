...
static void test_missing_kings(void) {
    Board board_no_white_king, board_no_black_king;
    ...
    assert(evaluate(&board_no_white_king) == 49000 && "Missing White king must return MATE_SCORE for Black");
    assert(evaluate(&board_no_black_king) == 49000 && "Missing Black king must return MATE_SCORE for White");
}
